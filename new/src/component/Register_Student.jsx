...
const Student = () => {
  ...
  const contiue = async(e) =>{
    ...
    try{
      const response = await axios.post("http://localhost:8080/project/phpbackened/controll.php", {
        studeid: studeid,
        what: "POST",
      }
      ,
      ...
      if(typeof response.data==='number'){
       ...
       const nownow = (response.data);
        ...
        toast.error("USER DOES'NT EXISTS. PLEASE MAKE SURE YOU ARE REGISTERED AS A SAINT IGNATIUS HIGH SCHOOL STUDENT");
      }
    }catch(error){
      toast.error(error);
      console.log(error)
